/**
 * VL Agents - True Multimodal Processing Agent
 * Handles Twitter AND Instagram, text + images + video via Gemini 2.0 Flash
 */
...
async function processItems(items) {
  ...
  for (const item of agentItems) {
    try {
      ...
      await db.run('UPDATE nodes SET metadata = ? WHERE id = ?', [
        JSON.stringify(updatedMeta), item.id
      ]);
...
async function main() {
  ...
  try {
    ...
    const nodes = await db.all(
      `SELECT * FROM nodes WHERE type IN ('twitter_tweet','instagram_post') ORDER BY timestamp DESC LIMIT 100`
    );
